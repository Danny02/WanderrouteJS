...
public class TrackSegment
{
     ...
     */
    public static enum caminarType
    {
        mullStop, pauseIndoor, vehicle, bike, run, walk, highway, flight, store, inside, carSet, car, photo, speedChange, undef
    };
	 ...
	 *
     */
    public static caminarType setCaminarType(double distance,
                                             List<TrackPoint> points)
    {
        ...
        if (avgSpeed > AIR_SPEED_FPS) {
            return caminarType.flight;
        }
        if (avgSpeed > HIGHWAY_SPEED_FPS) {
            return caminarType.highway;
        }
        if (avgSpeed > VEHICLE_CITY_SPEED_FPS) {
            return caminarType.vehicle;
        }
        if (avgSpeed > BIKE_SPEED_FPS) {
            return caminarType.bike;
        }
        if (avgSpeed > RUN_SPEED_FPS) {
            return caminarType.run;
        }
        if (avgSpeed > WALK_SPEED_FPS) {
            return caminarType.walk;
        }
        if (avgSpeed > STORE_SPEED_FPS) {
            return caminarType.store;
        }
        if (avgSpeed > 0.0) {
            return caminarType.mullStop;
        }
    ...
    public static double getTotalDistance(List<TrackPoint> points)
    {
        ...
        for (TrackPoint pt : points) {
            if (pt.isDistanceSet()) {
                totalDistanceTravelled += pt.getDistance().getFeetToNext();
            }
        }
    ...
    /**
     * TODO: use getTimeSpan() Gets average speed across track.
     * <p/>
	 ...
	 *
     */
    public static double speedOverTime(List<TrackPoint> pts, long startOffset,
                                       long timespan)
    {
	 ...
	 *
     */
    public static List<TrackSegment> splitIntoSegments(List<TrackPoint> points,
                                                       long timespan)
    {
    ...
    /**
     * find gaps in space or time describe pause append to end of trkseg print
     * out list of trksegs and the pauses inbetween
	 *
     */
    /**
	 ...
	 *
     */
    public static void shift(TrackSegment prev, TrackSegment middle,
                             TrackSegment next)
    {
	 ...
	 *
     */
    public static void shift(final TrackSegment middle, final TrackSegment next,
                             int fromEnd)
    {
        if (fromEnd > 0) {
	 ...
	 *
     */
    public static void shift(int fromStart, final TrackSegment prev,
                             final TrackSegment middle)
    {
        if (fromStart > 0) {
    ...
    /**
     * Given a list of track segments, merge adjacent segments that are similar
     * enough.
     * <p/>
	 ...
	 *
     */
    public static List<TrackSegment> mergeLikeSegments(
            List<TrackSegment> allSegs)
    {
	 ...
	 *
     */
    public static double getMovingAverage(long secondsMargin, int pos,
                                          List<TrackPoint> points)
    {
    ...
    /**
     * Returns position in list when gps points were not recorded, or -1 if none
     * occurred
     * <p/>
     * @param points           all points
     * @param secondsTolerance how long a time gap to look for
     * <p/>
     * @return position in list when positions were not recorded for
     *         timeTolerance time, or -1 if no gaps
	 *
     */
    public static int didTimePauseOccur(int fromPos, List<TrackPoint> points,
    ...
    /**
     * Returns position in list when speed changed dramatically, or -1 if none
     * occurred
     * <p/>
     * @param points       all points
	 ...
	 *
     */
    public static int didSpeedChangeOccur(int pos, List<TrackPoint> points,
                                          double fpsTolerance)
    {
    ...
    /**
     * returns whether I was mulling about in the same area for a period of
     * time.
     * <p/>
     ...
     */
    public static boolean amIMullingAbout(int pos, List<TrackPoint> points,
                                          double feetTolerance,
                                          double secondsTolerance)
    {
    ...
    /**
     * Returns a list of points surrounding one at pos depending on how far
     * apart they occurred.
     * <p/>
	 ...
	 *
     */
    public static List<TrackPoint> getTimespan(int pos, long secondsMargin,
                                               List<TrackPoint> pts)
    {
        TrackPoint start, end;
        int startPos = 0;
    ...
    /**
     * Returns a list of points surrounding one at pos depending on whether they
     * are with fettMargin.
     * <p/>
     * @param pos        which point to center on
	 ...
	 *
     */
    public static List<TrackPoint> getDistanceSpan(int pos, double feetMargin,
                                                   List<TrackPoint> pts)
    {
        TrackPoint start, end;
        int startPos = 0;
	 ...
	 *
     */
    public static String getAllSpeedChanges(List<TrackPoint> points,
                                            double fpsChangeTolerance)
    {
    ...
    /**
     * Splits list of TrackPoints into a list of 3 TrackSegments or 0
     * TrackSegments if no mulling about in one place occurred. The 3 items in
     * the list returned are: 0) TrackSegment of points before mulling about in
     * one place 1) TrackSegment of two points where the mulling about in one
     * place occurred 2) TrackSegment of points after the mulling about in one
     * place.
     * <p/>
     * @param points all points
     * <p/>
     * @return list of TrackSegments split based upon where mulling about in one
     *         place occurred, or a 0-item list if no split occurred
	 *
     */
    public static List<TrackSegment> splitListByMull(List<TrackPoint> pts,
                                                     double feetTolerance,
    ...
    /**
     * Splits list of TrackPoints into a list of 3 TrackSegments or 0
     * TrackSegments if no speed change occurred. The 3 items in the list
     * returned are: 0) TrackSegment of points before speed change 1)
     * TrackSegment of two points where the speed change occurred 2)
     * TrackSegment of points after speed change.
     * <p/>
     * @param points all points
     * <p/>
     * @return list of TrackSegments split based upon speed changes, or a 0-item
     *         list if no split occurred
	 *
     */
    public static List<TrackSegment> splitListBySpeedChange(
            List<TrackPoint> points, double fpsChangeTolerance)
    {
        int curr = 0;
        List<TrackSegment> lists = new ArrayList<TrackSegment>();
    ...
    /**
     * Splits list of TrackPoints into a list of 3 TrackSegments or 0
     * TrackSegments if no interruptions in time occurred. The 3 items in the
     * list returned are: 0) TrackSegment of points before time interruption 1)
     * TrackSegment of two points where the time interruption occurred 2)
     * TrackSegment of points after time interruption.
     * <p/>
     * @param points all points
     * <p/>
     * @return list of TrackSegments split based upon time interruptions, or a
     *         0-item list if no split occurred
	 *
     */
    public static List<TrackSegment> splitListByTimeInterruption(
            List<TrackPoint> points, int secondsGapTolerance)
    {
        int curr = 0;
        List<TrackSegment> lists = new ArrayList<TrackSegment>();
    ...
    private static String formatTime(TrackSegment ts)
    {
        ...
        if (timespan < TrackSegment.LMINUTE) {
            return "for less than a minute";
        }
        if (timespan < TrackSegment.LMINUTE * TrackSegment.LMINUTE) {
            return String.format("for %d minutes", timespan / TrackSegment.LMINUTE);
        }
    ...
    public static String formatDistance(double totalDistance)
    {
        if (totalDistance < 400.0) {
            return String.format("%.3f %s", totalDistance, Trip.USE_KM ? "meters" : "feet");
        }
